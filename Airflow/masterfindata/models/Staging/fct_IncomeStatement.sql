...
WITH source AS (
    ...
    WHERE 
        stg_pre.STMT = 'IS'  -- Filter only Balance Sheet Data
),

final AS (
    SELECT DISTINCT
        {{ dbt_utils.generate_surrogate_key(['src.ADSH', 'src.VALUE']) }} AS COMP_FCT_SK,  -- Fact Table SK
        dim_company.COMPANY_SK,
        dim_filings.FILINGS_SK,
        src.FiledDate AS FiledDate, 
        src.VALUE,
        src.ADSH AS ADSH_KEY,
        'DBT_USER' AS Created_By,  -- Placeholder for tracking
        'IS' AS STMT,  -- Marking as Balance Sheet Data
    ...
    LEFT JOIN {{ ref('dim_filings') }} dim_filings 
        ...
        AND src.FiledDate = dim_filings.FiledDate       -- Matching on FILED date
    LEFT JOIN {{ ref('dim_date') }} dim_date 
        ON TRY_TO_DATE(src.FiledDate::STRING, 'YYYY-MM-DD') = dim_date.Full_DT
)
